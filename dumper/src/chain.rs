...
#[inline]
pub fn get_pointer_chain_address<P: VirtualMemoryRead + ProcessInfo>(proc: &P, chain: &str) -> Option<usize> {
    let mut parts = chain.split(&['[', ']', '+', '@']);
    let name = parts.next()?;
    let index = parts.next()?.parse().ok()?;
    let offset = parts.next_back()?.parse().ok()?;
    let elements = parts.skip(1).map(|s| s.parse());

    let mut address = match find_base_address(proc, name, index) {
        Some(a) => a,
        None => {
    ...
    let address = address.checked_add_signed(offset)?;
